...
var DiagnosticsAdapter = /** @class */ (function (_super) {
    ...
    DiagnosticsAdapter.prototype._doValidate = function (resource) {
        var _this = this;
        this._worker(resource).then(function (worker) {
            ...
            var promises = [];
            var _a = _this._defaults.getDiagnosticsOptions(), noSyntaxValidation = _a.noSyntaxValidation, noSemanticValidation = _a.noSemanticValidation, noSuggestionDiagnostics = _a.noSuggestionDiagnostics;
    ...
    DiagnosticsAdapter.prototype._convertDiagnostics = function (resource, diag) {
    ...
    DiagnosticsAdapter.prototype._convertRelatedInformation = function (resource, relatedInformation) {
        var _this = this;
        if (relatedInformation === undefined)
            return undefined;
        return relatedInformation.map(function (info) {
            var relatedResource = info.file === undefined ? resource : monaco.Uri.parse(info.file.fileName);
            var _a = _this._offsetToPosition(relatedResource, info.start), startLineNumber = _a.lineNumber, startColumn = _a.column;
            var _b = _this._offsetToPosition(relatedResource, info.start + info.length), endLineNumber = _b.lineNumber, endColumn = _b.column;
            return {
...
var CodeActionAdaptor = /** @class */ (function (_super) {
    ...
    CodeActionAdaptor.prototype.provideCodeActions = function (model, range, context, token) {
        ...
        var resource = model.uri;
        return this._worker(resource).then(function (worker) {
            var start = _this._positionToOffset(resource, { lineNumber: range.startLineNumber, column: range.startColumn });
            var end = _this._positionToOffset(resource, { lineNumber: range.endLineNumber, column: range.endColumn });
            var formatOptions = FormatHelper._convertOptions(model.getOptions());
            var errorCodes = context.markers.filter(function (m) { return m.code; }).map(function (m) { return m.code; }).map(Number);
            return worker.getCodeFixesAtPosition(resource.toString(), start, end, errorCodes, formatOptions);
        }).then(function (codeFixes) {
            return codeFixes.filter(function (fix) {
    ...
    CodeActionAdaptor.prototype._tsCodeFixActionToMonacoCodeAction = function (model, context, codeFix) {
        ...
        var edits = codeFix.changes.map(function (edit) { return ({
            ...
            edits: edit.textChanges.map(function (tc) { return ({
                range: _this._textSpanToRange(model.uri, tc.span),
                text: tc.newText
            }); })
        }); });
        var action = {
